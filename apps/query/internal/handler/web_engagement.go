...
func (h *ProductHandler) WebEngagement(c fiber.Ctx) error {
	...
	// These six reads are independent — none uses another's result — so they run
	// concurrently. Each func writes only its own variable(s); derived values
	// (the scroll slice) are built afterward.
	downloads := []fiber.Map{}    // 1) Top downloaded files.
	outbound := []fiber.Map{}     // 1b) Outbound link clicks by destination domain.
	customEvents := []fiber.Map{} // 2) Custom events (anything explicitly track()'d).
	timeOnPage := []fiber.Map{}   // 2b) Time on page: average foreground engagement per page.
